...
#scenario_list <- "ASSss3_HCR4_REClow_INNvar_OERnaq"
...
for (cs in scenario_list){
 ...
 # tt <- c(1:42,44:834,839:1000)
  ...
  for (i in 1:ni){ #1:ni
    print(paste("     iter",i))
    if (file.exists(file.path(res.dir, "output_iters0.064", paste("out2018_",cs,"_",i,".RData",sep="")))){
      load(file.path(res.dir, "output_iters0.064", paste("out2018_",cs,"_",i,".RData",sep="")))
    ...
    } else stop("Missing file:", file.path(res.dir, "output_iters0.064", paste("out2018_",cs,"_",i,".RData",sep="")))
  }
  ...
  save( out.bio, out.flt, out.adv, out.risk, #out.fltstk, 
        file=file.path(res.dir, "output_scenarios", paste("results2018_",cs,"_Ftgt0.064.RData",sep="")))
}
