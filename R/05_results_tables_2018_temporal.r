...
load( "./temporal_output/res_bio_all2018.RData")    # dat.bio.q
# dat.bio.q$stock <- NULL

load( "./temporal_output/res_eco_all2018.RData")    # dat.eco.q
dat.eco.q <- dat.eco.q[,c("year","scenario",paste("effort",qs,sep="_"))]

load( "./temporal_output/res_adv_all2018.RData")    # dat.adv.q
...
write.table( out, file=file.path("temporal_output","stats_byyr2018.csv"), dec = ".", sep = ";",
             row.names = FALSE)
...
for (cs in scenario_list){
  ...
  obj <- perfInd.pil( obj.bio="out.bio", , obj.adv="out.adv", 
                      scenario=cs, file.dat=file.path("temporal_output/output_scenarios",paste("results2018_",cs,".RData",sep="")),
...
# Save data
write.table( out, file="./temporal_output/stats2018.csv", dec = ".", sep = ";",
             row.names = FALSE)
