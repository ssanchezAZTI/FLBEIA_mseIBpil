...
scenario_list<-unique(sapply(gsub('.{6}$', '', files),function(x){paste(strsplit(x,"_")[[1]][2:7],collapse="_")}))
...
dat.bio.q <- dat.bio.q %>%separate(scenario, into = c("Ass", "Rule", "Rec", "INN", "OER","Ftgt"), sep = "_",  remove=FALSE)
...
dat.eco.q <- dat.eco.q %>%separate(scenario, into = c("Ass", "Rule", "Rec", "INN", "OER","Ftgt"), sep = "_",  remove=FALSE)
...
dat.adv.q <- dat.adv.q %>%separate(scenario, into = c("Ass", "Rule", "Rec", "INN", "OER","Ftgt"), sep = "_",  remove=FALSE)
